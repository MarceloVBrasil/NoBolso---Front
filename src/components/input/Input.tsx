...
export default function Input({ type, placeholder, classname, onChange, selectOptions, label, value, dataTestId }: { type: string, placeholder: string, classname?: string, onChange: ChangeEventHandler, selectOptions?: any, label?: string, value?: string, dataTestId?: string }) {
    ...
    switch (type) {
        case 'text':
            return (
                <input data-testid={dataTestId} placeholder={placeholder} type={'text'} className={`bg-[#eceff1] p-4 w-full outline-[#CCC] ${classname}`} onChange={onChange} />
            )
        case 'senha':
            return (
                <div data-testid={dataTestId} className='relative w-full'>
                    <Lock width="20" height="20" classname='absolute top-5 left-4' />
                    <input name='senha' type={!mostrarSenha ? 'password' : 'text'} placeholder={placeholder} className={`bg-[#eceff1] indent-6 p-4 w-full outline-[#CCC] ${classname}`} onChange={onChange} />
        ...
        case 'email':
            return (
                <div data-testid={dataTestId} className='w-full relative'>
                    <Email classname='absolute top-5 left-4' />
                    <input name='email' type="text" placeholder={placeholder} className={`indent-6 bg-[#eceff1] p-4 w-full outline-[#CCC] ${classname}`} onChange={onChange} />
                </div>
            )
        case 'money':
            return (
        ...
        case 'select':
            return (
                <div className='flex flex-col relative gap-1 w-44'>
                    ...
                    <select data-testid={dataTestId} className={`${classname} p-2 w-full border outline-[#CCC] h-10 relative bottom-1 text-cinzaEscuro`} onChange={onChange}>
        ...
        case 'novaCategoria':
            return (
                <input name='nova-categoria' data-testid={dataTestId} value={value} placeholder={placeholder} type={'text'} className={`border p-4 outline-[#CCC] ${classname}`} onChange={onChange} />
            )
        case 'user':
            return (
                <div data-testid={dataTestId} className='w-full relative'>
